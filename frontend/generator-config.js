    ...
    // or, for direct-to-ML-server setups, via window.BEAULIX_API_KEY set in a
    // server-rendered script block before this file loads.  In local dev the
    // variable is absent and the backend skips the check automatically.
    // NOTE: proxying through Firebase Functions (functions/index.js) is the
    // preferred path — it keeps the key entirely server-side and hides the ML
    // server URL from clients.  See the "Proxy the ML API" note in the repo docs.
    ...
    function updateStep2RecsFromAPI(recs) {
      if (!recs || (!recs.recommended_brand_style && !recs.recommended_aspect_ratio && !recs.recommended_output_type)) {
        visualStrategySection.style.display = 'none';
        return;
      }
      ...
      visualStrategySection.style.display = 'block';
    }
    ...
    function updateGenerateButtonState() {
      const analysisComplete = analysisResults.style.display === 'block';
    ...
    async function runGeneration(payload) {
      generateCreativeSpinner.style.display = 'block';
      ...
      try {
        ...
        previewBox.style.display = 'none'; generatedOutput.style.display = 'flex'; downloadBtn.disabled = true;
      } finally {
        generateCreativeSpinner.style.display = 'none'; generateCreativeBtn.disabled = false;
        generationProgress.style.display = 'none'; progressBar.style.width = '0%';
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      humanOptionsSection.style.display = 'none';
      ...
      includeHumanFace.addEventListener('change', () => { humanOptionsSection.style.display = includeHumanFace.checked ? 'block' : 'none'; });
      ...
      document.querySelectorAll('input[name="output-type"]').forEach(radio => {
        radio.addEventListener('change', () => { document.getElementById('duration-group').style.display = radio.value==='video'?'block':'none'; updateGenerateButtonState(); onStep2SelectionChange(); });
      });
    ...
    analyzeBtn.addEventListener('click', async function() {
      ...
      loadingStages.style.display = 'block'; analysisResults.style.display = 'none'; analyzeBtn.disabled = true;
      ...
      } catch (error) { showToast(`Error: ${error.message}`,'error'); analysisResults.style.display='none'; }
      finally {
        loadingStages.style.display='none'; analyzeBtn.disabled=false;
    ...
    generateCreativeBtn.addEventListener('click', async function() {
      if (analysisResults.style.display==='none') { showToast('Please complete marketing profile analysis first','error'); return; }
      const productTypeVal = productType.value.trim();
...
// ── Copy buttons (moved from inline onclick) ──
